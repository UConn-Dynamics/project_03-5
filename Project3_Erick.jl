...
# ╔═╡ ed3012f0-4b4b-11f1-bc09-259d7a60cdde
begin
    using LinearAlgebra, ForwardDiff, DifferentialEquations
	using Plots

    # PARAMETERS
       const m1, m2 = 0.1, 0.3      #kg
    const L, k, g = 0.4, 10.0, 9.81
    const I1 = 1e-6              
    const I2 = (1/12) * m2 * L^2
    
    # Mass Matrix M
    const M = Diagonal([m1, m1, I1, m2, m2, I2])
    ...
    function get_gamma(q, qdot)
        # gamma = - (Cq_dot * qdot)
        tmp_vel_func(vec_q) = Cq(vec_q) * qdot
        return -ForwardDiff.jacobian(tmp_vel_func, q) * qdot
    end

    # 3. STABILIZED AUGMENTED SOLVER
    function solve_augmented_dynamics(q, qdot; dt=0.01)
       
        α, β = 1/dt, sqrt(2)/dt
        
        _C, _Cq = C(q), Cq(q)
        γ_stab = get_gamma(q, qdot) - 2α*(_Cq * qdot) - β^2 * _C
        
        # Form Augmented Matrix
        A_aug = [M _Cq'; _Cq zeros(4,4)]
        b_aug = [Q_ext(q); γ_stab]
        
        res = A_aug \ b_aug
        return res[1:6], res[7:10] # [qddot, λ]
    end

        # 4. SIMULATION EXECUTION
    function ode_system!(du, u, p, t)
        q, qdot = u[1:6], u[7:12]
        qddot, λ = solve_augmented_dynamics(q, qdot)
        du[1:6], du[7:12] = qdot, qddot
    end

    # Initial Conditions: Block at 0.05m, Bar at 45 degrees
    q0 = [0.05, 0.0, 0.0, 0.05 + (L/2)*cos(π/4), (L/2)*sin(π/4), π/4]
    u0 = vcat(q0, zeros(6))
    tspan = (0.0, 8.0)

    prob = ODEProblem(ode_system!, u0, tspan)
    sol = solve(prob, Rodas5(), reltol=1e-7, abstol=1e-7)

   
    # 5. DATA EXTRACTION & POST-PROCESSING
   
    t_steps = sol.t
    q_hist = [u[1:6] for u in sol.u]
    v_hist = [u[7:12] for u in sol.u]
    
    # Re-calculate Forces and Energy
    force_data = [solve_augmented_dynamics(u[1:6], u[7:12])[2] for u in sol.u]
    
    function get_total_energy(q, v)
        x1, y1, _, x2, y2, θ2 = q
        vx1, vy1, _, vx2, vy2, _ = v
        # T (Kinetic) + V (Potential)
        T = 0.5*m1*(vx1^2+vy1^2) + 0.5*m2*(vx2^2+vy2^2) + 0.5*I2*(v[6]^2)
        V = 0.5*k*x1^2 + m1*g*y1 + m2*g*y2
        return T + V
    end
    energy_hist = [get_total_energy(q_hist[i], v_hist[i]) for i in 1:length(t_steps)]

   
    # 6. FINAL PLOTTING & ANIMATION
    
    # Plot 1: Constraint Forces
    p1 = plot(t_steps, [f[1] for f in force_data], label="Track Normal Force (N)")
    plot!(p1, t_steps, [f[3] for f in force_data], label="Pin Force X (N)")
    plot!(p1, t_steps, [f[4] for f in force_data], label="Pin Force Y (N)", 
          title="Constraint Forces", xlabel="Time (s)", ylabel="Force")

    # Plot 2: Energy Conservation (Verification)
    p2 = plot(t_steps, energy_hist .- energy_hist[1], title="Energy Drift (Should be ~0)",
              xlabel="Time (s)", ylabel="ΔE (J)", label="Energy Error")

    # Animation
    anim = @animate for i in 1:4:length(sol.t)
        q_i = q_hist[i]
        x1, y1, _, x2, y2, θ2 = q_i
        
        # Geometry for drawing
        px, py = x1, y1
        tx, ty = x1 + L*cos(θ2), y1 + L*sin(θ2)
        
        plot(xlims=(-0.2, 0.8), ylims=(-0.6, 0.4), aspect_ratio=:equal, grid=true, title="Project 03 Animation")
        hline!([0], color=:black, alpha=0.2, label="") # Track
        plot!([0, x1], [0, 0], color=:red, lw=2, label="Spring") # Spring
        scatter!([x1], [y1], markershape=:rect, markersize=12, label="Block")
        plot!([px, tx], [py, ty], lw=6, color=:black, label="Compound Bar")
        scatter!([x2], [y2], markersize=4, color=:yellow, label="Bar COM")
    end

    # Display plots
    display(plot(p1, p2, layout=(2,1), size=(800, 600)))
    gif(anim, "final_simulation.gif", fps=30)
end
...
manifest_format = "2.0"
project_hash = "c72288272c4c7de9f731e65c386f8b8aae9c6946"
